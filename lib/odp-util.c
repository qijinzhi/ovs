...
/*
int
odp_tt_from_string(const char *s, struct ofputil_tt_flow_mod_msg *mod)
{
	char *s_;
	s_ = strtok(s, ",");
	if (!s_) {
	    mod->port = (uint8_t)atoi(s_);
	} else {
		return -1;
	}
	s_ = strtok(NULL, ",");
	if (!s_) {
		mod->etype = (uint8_t)atoi(s_);
	} else {
		return -1;
	}
	s_ = strtok(NULL, ",");
	if (!s_) {
		mod->flow_id = (uint8_t)atoi(s_);
	} else {
		return -1;
	}
	s_ = strtok(NULL, ",");
	if (!s_) {
		mod->scheduled_time = (uint32_t)atoi(s_);
	} else {
		return -1;
	}
	s_ = strtok(NULL, ",");
	if (!s_) {
		mod->period = (uint32_t)atoi(s_);
	} else {
		return -1;
	}
	s_ = strtok(NULL, ",");
	if (!s_) {
		mod->buffer_id = (uint32_t)atoi(s_);
	} else {
		return -1;
	}
	s_ = strtok(NULL, ",");
	if (!s_) {
		mod->pkt_size = (uint32_t)atoi(s_);
	} else {
		return -1;
	}
	return 0;
